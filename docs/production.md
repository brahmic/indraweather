...
Отдельный volume `indra_cloud_animation`, подключённый по пути
`/var/lib/indra/cloud-animation`, хранит кадры диагностической анимации
`/clouds`. Она независима от ИК-анимации `/weather` и не смешивает дневные типы
облаков с ночным туманом.

Volume `indra_personal_animation`, подключённый по пути
`/var/lib/indra/personal-animation`, хранит короткий кэш персональных MP4 для
пользователей, настроивших `/map`. Само изменение карты ничего не публикует.
Задание создаётся только после явного `/weather` или `/clouds`; если к окончанию
сборки пользователь изменил охват, ролик не отправляется. Никаких обязательных
новых переменных окружения для этой функции нет: значения по умолчанию уже есть
в `.env.example`.
