...
const envSchema = z.object({
  ...
  SCHEDULE_PREPARE_MINUTES: z.coerce.number().int().min(1).max(60).default(10),
  ...
  WEATHER_FALLBACK_MAX_AGE_HOURS: z.coerce.number().int().min(1).max(48).default(12),
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    schedulePrepareMinutes: parsed.SCHEDULE_PREPARE_MINUTES,
    ...
    weatherFallbackMaxAgeHours: parsed.WEATHER_FALLBACK_MAX_AGE_HOURS,
    satellite: {
